...
wavelength=0.055165;
aecimagedisp=-aecimage.*wavelength.*100./(4*pi);
...
randrow=width(1:n);
for i=1:n
    aecsample=aecimagedisp(randline(i):randline(i)+lbox-1,randrow(i):randrow(i)+rbox-1);
...
samplesd(A)=0;
sd=mean(samplesd);
% figure,imagesc(aecimagedisp);colorbar;colormap(jet);title(aecsd);
saveas(gcf,'aecdisp','png')
% figure,imagesc(imagedisp);colorbar;colormap(jet);title(sd);
saveas(gcf,'disp','png')
