#!/usr/bin/env python3
#############################################################
# Stack processing using GAMMA for StaMPS SBAS processing   #
...
dinsar_script = """m_slc=$slc_dir/$m_date/$m_date.rslc
s_slc=$slc_dir/$s_date/$s_date.rslc
m_par=$m_slc.par
s_par=$s_slc.par
...
##################################################################################################################
offset_pwr $m_slc $s_slc $m_par $s_par $MS_off $m_date-$s_date.offs $m_date-$s_date.off.snr 32 32 $m_date-$s_date.offsets 1 500 500 7.0 2
...
def gen_ifg_pairs(slc_dir, super_master, bperp_min, bperp_max, delta_T_min,
    ...
    slaves_par = []
    for slave in slaves:
        slc_path = os.path.join(slc_dir, slave, slave + '.rslc')
        par_path = slc_path + '.par'
        slaves_slc.append(slc_path)
        slaves_par.append(par_path)
    sm_slc = os.path.join(slc_dir, super_master, super_master + '.rslc')
    sm_par = sm_slc + '.par'
    # write slc_tab
