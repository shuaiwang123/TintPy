...
def load_data(txt):
    ...
    dates = np.asarray(content[0, 3:], np.int64)
    dates = [
    ...
    return lons, lats, vels, ts, dates


def random_downsample(lons, lats, vels, ts, rate):
    """downsample data"""
    point_num = lons.shape[0]
    index = random.sample(range(point_num), int(point_num * rate) + 1)
    return lons[index], lats[index], vels[index], ts[index, :]
...
def write_kml(lons, lats, vels, ts, dates, cb, dygraph_file, scale, out_file):
...
def main():
    ...
    try:
        lons, lats, vels, ts, dates = load_data(ts_file)
        if rate < 1:
            lons, lats, vels, ts = random_downsample(lons, lats, vels, ts, rate)
    except:
    ...
    print('write kml...')
    write_kml(lons, lats, vels, ts, dates, cb, js_file, scale, out_file)
    print('done.')
    # delete files
    print('delete files...')
    del_files(dir_name)
